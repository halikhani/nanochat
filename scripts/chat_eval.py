...
def run_categorical_eval(task_object, tokenizer, model, batch_size, max_problems=None):
    ...
    for i in range(ddp_rank, num_batches, ddp_world_size):
        ...
        # prepare the batch of problems and pad/collate them if needed (different lengths)
        conversations = [task_object[j] for j in range(i0, i1)]
        ...
        # get the logits for the whole batch of conversations in parallel (for efficiency)
        with torch.no_grad():
            logits = model(prompt_ids) # (B, T, V) where V is vocab size
        
        # basically we dont sample the model, only get the logits and look at the desired ones (letters for the answer)
        ...
        for idx, conversation in enumerate(conversations):
            ...
            # conversation = {
            #     'letters': ['A', 'B', 'C', 'D'],  # Available choices
            #     'query': 'What is 2+2?',
            #     'choices': {
            #         'A': '3',
            #         'B': '4',
            ...
            for letter in letters:
                if not letter in letter_to_id_cache:
                    ...
                    assert len(encoded_letter) == 1, "Letter encoding should be a single token"
                    # example: letter = 'A' -> encoded_letter = [101]
                    letter_to_id_cache[letter] = encoded_letter[0]
                letter_ids.append(letter_to_id_cache[letter])
            # e.g., outcome: letter_ids = [101, 102, 103, 104]  # A, B, C, D

            # focus logits just down to the answer position and the available letters of the answer
            answer_pos = answer_time_positions[idx]
            focus_logits = logits[idx, answer_pos, letter_ids] # shape is (4,) for each letter in the batch for A, B, C, D
...
def run_chat_eval(task_name, model, tokenizer, engine, batch_size=1,
