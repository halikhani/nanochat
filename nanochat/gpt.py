...
class MLP(nn.Module):
    ...
    def forward(self, x):
        x = self.c_fc(x)
        x = F.relu(x).square() # squaring results element-wise
        x = self.c_proj(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, cos_sin, kv_cache):
        # Two residuals: attention then MLP.
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.rotary_seq_len = config.config.sequence_len * 10 # 10X over-compute
        head_dim = config.n_embd // config.n_head
        cos, sin = self._precompute_rotary_embeddings(self.rotary_seq_len, head_dim)
        # storeing tensors on the module without treating them as learnable params
        self.register_buffer("cos", cos, persistent=False) # persistent=False means it's not saved to the checkpoint and will be recomputed on the load
    ...
    def _precompute_rotary_embeddings(self, seq_len, head_dim, base=10000, device=None):
        ...
        # stride the channels
        channel_range = torch.arange(0, head_dim, 2, dtype=torch.float32, device=device) #Builds channel_range = [0, 2, 4, …] up to head_dim
        inv_freq = 1.0 / (base ** (channel_range / head_dim)) # giving slower frequencies for higher channel indices
        # stride the time steps
        t = torch.arange(seq_len, dtype=torch.float32, device=device) # t = [0, 1, 2, …, seq_len-1] represents token positions.
        # calculate the rotation frequencies at each (time, channel) pair
        freqs = torch.outer(t, inv_freq)
        cos, sin = freqs.cos(), freqs.sin()
        cos, sin = cos.bfloat16(), sin.bfloat16() # keep them in bfloat16
        cos, sin = cos[None, :, None, :], sin[None, :, None, :] # Adds singleton batch and head dimensions ([1, seq_len, 1, head_dim/2]) so later rotary ops can broadcast across batch and heads without reshaping.
        return cos, sin
